...
Web developers use the Long Tasks or LoAF API to identify long-running tasks that affect responsiveness and UI updates.
...
      // Simulate long task by using setTimeout
      setTimeout(() => {
        postMessage('Worker task completed after delay');
      }, 2000);
    }
};
```
